...
export default function ImageCard(props) {
   ...
   const [url, setUrl] = useState("");
    ...
    fetch(`https://face-attributes-search.herokuapp.com/faces/api/v1/search/${props.image_id}`, option)
        .then(res => res.json())
        .then(data => {
            console.log(data)
            if( data === 'OK')
    ...
    const getImg = (image) =>{
        ...
        fetch(`https://face-attributes-search.herokuapp.com/faces/api/v1/search/${image}`)
            .then(res => res.json())
            .then(data => {
